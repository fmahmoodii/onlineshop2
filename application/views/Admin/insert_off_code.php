		...
		<div class="row" style="margin-top: 50px; margin-bottom: 60px">
			...
			<div class="col-md-8">
				<div style="padding: 20px;border-radius: 10px; border: 1px solid #ccc">
					<form method="post" action='<?php echo base_url('admin/insert_off_code') ?>' onsubmit="return validateForm()" >
						...
						<div class="form-group">
							...
							<input required autocomplete="off" id="code" name="code"
								   class="form-control" type="text" placeholder="کد تخفیف"
						 ...
						 overflow-y: scroll;"></div>
							...
							<?php echo form_error('code','<span id="code_error" style="color: red">','</span>') ?>
						</div>

						<div class="form-group">
							<label for="mablagh" class="required">مبلغ تخفیف:</label><br>
							<input required autocomplete="off" id="mablagh" name="mablagh"
								   class="form-control positive" type="number" min="0" placeholder="مبلغ تخفیف"
								   value="<?php echo set_value('mablagh'); ?>">
						...
						<div class="form-group start_date" style="display: inline-block; width: 270px">
							...
							<?php echo form_error('start_date','<span style="color: red">','</span>') ?><br>
							<input required readonly id="start_date" name="start_date"
						...
						<div class="form-group end_date" style="display: inline-block; width: 270px">
							...
							<?php echo form_error('end_date','<span style="color: red">','</span>') ?><br>
							<input required readonly id="end_date" name="end_date"
								   class="form-control" placeholder="تاریخ پایان"
						...
						<div class="form-group">
							...
							<input required autocomplete="off" id="min_price" name="min_price"
								   class="form-control positive" type="number" min="0" placeholder="حداقل خرید"
								   value="<?php echo set_value('min_price'); ?>">
...
<!--								<option selected value="">--><?php //echo "انتخاب کنید.."; ?><!--</option>-->
								...
								<?php foreach ($products as $p){ ?>
									<option value="<?php echo $p->id?>" <?= set_select('products', $p->id, false) ?> ><?php echo $p->name?></option>
								<?php }?>
							</select>
...
<!--							<button type="button" id="deselectAll_p" class="btn btn-default">حذف همه</button>-->
    ...
    margin-right: 10px;">کاربران:</label>
							<br>
							<select multiple class="form-control col-md-4 users" id="users" name="users[]" >

<!--								<option selected value="">--><?php //echo "انتخاب کنید.."; ?><!--</option>-->

								<?php foreach ($register as $reg){ foreach ($profile as $prof){if($reg->id == $prof->user_id){ ?>
									<option value="<?php echo $reg->id?>" <?= set_select('users', $reg->id, false) ?> ><?php $x=$prof->name.' '.$prof->family; echo $x;?></option>
								<?php }}}?>
							</select>
