...
<?php if($this->session->userdata('id')){ ?>
	...
	<div class="container box" id="content">
		...
		<br>
		<br>
		<table id="category1_data" class="table table-bordered table-striped">
			<thead>
			<tr>
				<th>
...
<script>
	...
	$(document).ready(function () {
		var dataTable = $('#category1_data').DataTable({
			language: {
			...
			"order": [],
			"ajax": {
				url: "<?php echo base_url() . 'admin/category1_list'; ?>",
				type: "POST"
			},
			"columnDefs": [
				{
	...
	$(document).on('click', '#delete', function(e){
		var id = $(this).attr("id_cat1");
		$.ajax({
			url:"<?php echo base_url(); ?>admin/cat2_product",
			method:"POST",
			data:{id:id},
			success:function(data1)
			{
				var obj = JSON.parse(data1);
				if(obj.cat2 == 0 && obj.prd != 0){
					var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد '+obj.prd+' محصول در این دسته ویرایش می شوند');
				}
				if (obj.prd == 0 && obj.cat2 != 0){
					var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد '+obj.cat2+' زیرمجموعه حذف می شوند');
				}
				if(obj.cat2 != 0 && obj.prd != 0) {
					var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد ' + obj.cat2 + ' زیرمجموعه حذف و ' + obj.prd + ' محصول در این دسته ویرایش می شوند');
				}
				if(obj.cat2 == 0 && obj.prd == 0){
					var m = confirm('آیا از حذف این دسته اطمینان دارید؟')
				}
				if (m == true) {
					$.ajax({
						url: "<?php echo base_url(); ?>admin/delete_cat1",
						method: "POST",
						data: {id: id},
	...
	$(document).on('click', '#btn_edit', function (e) {
		...
		$.post('<?php echo base_url()?>admin/edit_cat1', {
				'id': id,
				'name_cat1': name,
			},
			function (data) {
	...
	$("#delete_selected").on("click", function () {
		...
		$.ajax({
			type: "POST",
			url:"<?php echo base_url(); ?>admin/cat2_product_2",
			data: {'ids': ids,},
			dataType: "html",
			cache: false,
			success: function(data1) {
				if (ids.length > 0) {
					var obj = JSON.parse(data1);
					if (obj.cat2 == 0 && obj.prd != 0) {
						var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد '+obj.prd+' محصول ویرایش می شوند');
					}
					if (obj.prd == 0 && obj.cat2 != 0) {
						var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد '+obj.cat2+' زیرمجموعه حذف می شوند');
					}
					if (obj.cat2 != 0 && obj.prd != 0) {
						var m = confirm('از حذف دسته اطمینان دارید؟\nتعداد ' + obj.cat2 + ' زیرمجموعه حذف و ' + obj.prd + ' محصول ویرایش می شوند');
					}
					if (obj.cat2 == 0 && obj.prd == 0) {
						var m = confirm('آیا از حذف این دسته اطمینان دارید؟')
					}
					if (m == true) {
						$.ajax({
							type: "POST",
							url: "<?php echo base_url(); ?>admin/delete_cats1_checked",
							data: {'ids': ids,},
							dataType: "html",
							cache: false,
							success: function (data) {
	...
	$("#btn_edit2").on("click", function () {
		...
		if(ids.length > 0) {
				...
				$.ajax({
					url:"<?php echo base_url(); ?>admin/edit_cats1_checked",
					method:"POST",
