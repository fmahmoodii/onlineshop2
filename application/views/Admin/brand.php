		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<label class="required">برند:</label><br>
					<input required autocomplete="off" id="name" class="form-control" type="text" placeholder="برند">
			...
			<div class="modal-content " style="background-color: #fff">
				...
				<div class="modal-body">
					<div>
						...
						<label class="required">برند:</label><br>
						<input required autocomplete="off" id="name_modal" type="text" class="form-control" placeholder="برند">
...
<script type="text/javascript" language="javascript">
	$(document).ready(function () {
		var dataTable = $('#brand_data').DataTable({
			...
			"order": [],
			"ajax": {
				url: "<?php echo base_url() . 'admin/brand_list'; ?>",
				type: "POST"
			},
			"columnDefs": [
				{
		...
		$('#name').keyup(function () {
			...
			function load_data(query) {
				$.ajax({
					url: "<?php echo base_url(); ?>admin/search_brand",
					method: "POST",
					data: {query: query,},
			...
			if (name != '') {
				load_data(name);
				$.ajax({
					url: "<?php echo base_url(); ?>admin/check_brand",
					method: "POST",
					data: {name: name},
					success: function (data) {
						$('#error').html(data);
	...
	$(document).on('click', '#delete', function () {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>admin/check_brand_in_tables",
			method: "POST",
			data: {id: id},
			success: function (data) {
				...
				if (m2 == true) {
					$.ajax({
						...
						success: function (data) {
							if(data.includes(1)){
								$('#brand_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#insert', function (e) {
		...
		$.post('<?php echo base_url()?>admin/brand_validation', {
				'name': name,
			},
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/insert_brand', {
							'name': name,
	...
	$(document).on('click', '#copy', function (e) {
		var id = $(this).attr("id_brand");
		$.post('<?php echo base_url()?>admin/brand_copy', {
				'id': id,
			},
			function () {
				$('#brand_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#btn_edit', function (e) {
		...
		$.post('<?php echo base_url()?>admin/brand_validation', {
			...
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/edit_br', {
							'id': id,
							'name': name,
						},
						function (data) {
		...
		$.post('<?php echo base_url();?>admin/edit_br', {'id': id, 'name': name},
			function (data) {
				if (data == 1) {
					$("#name_" + id).text(name);
