...
<?php if ($this->session->userdata('id')) { ?>
	...
	<div class="container box" id="content">
		...
		<br>
		<br>
		<table id="jens_data" class="table table-bordered table-striped">
			<thead>
			<tr>
				<th >icon</th>
				<th >جنس</th>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<label class="required">جنس:</label><br>
					<input required autocomplete="off" id="name" class="form-control" type="text" placeholder="جنس">
			...
			<div class="modal-content " style="background-color: #fff">
				...
				<div class="modal-body">
					<div>
						...
						<label class="required">جنس:</label><br>
						<input required autocomplete="off" id="name_modal" type="text" class="form-control" placeholder="جنس">
...
<script type="text/javascript" language="javascript">
	$(document).ready(function () {
		var dataTable = $('#jens_data').DataTable({
			language: {
			...
			"order": [],
			"ajax": {
				url: "<?php echo base_url() . 'admin/jens_list'; ?>",
				type: "POST"
			},
			"columnDefs": [
				{
		...
		$('#name').keyup(function () {
			...
			function load_data(query) {
				$.ajax({
					url: "<?php echo base_url(); ?>admin/search_jens",
					method: "POST",
					data: {query: query,},
			...
			if (name != '') {
				load_data(name);
				$.ajax({
					url: "<?php echo base_url(); ?>admin/check_jens",
					method: "POST",
					data: {name: name},
					success: function (data) {
						$('#error').html(data);
	...
	$(document).on('click', '#delete', function () {
		var id = $(this).attr("id_jens");
		$.ajax({
			url: "<?php echo base_url(); ?>admin/check_jens_in_tables",
			method: "POST",
			data: {id: id},
			success: function (data) {
				...
				if (m2 == true) {
					$.ajax({
						url: "<?php echo base_url(); ?>admin/delete_jens",
						method: "POST",
						data: {id: id},
						success: function (data) {
							if(data.includes(1)){
								$('#jens_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#insert', function (e) {
		...
		$.post('<?php echo base_url()?>admin/jens_validation', {
				'name': name,
			},
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/insert_jens', {
							'name': name,
						...
						function (data) {
							if (data.includes(1)) {
								...
								//window.location.href = "<?php echo base_url(); ?>admin/jens";
	...
	$(document).on('click', '#copy', function (e) {
		var id = $(this).attr("id_jens");
		$.post('<?php echo base_url()?>admin/jens_copy', {
				'id': id,
			},
			function () {
				$('#jens_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#btn_edit', function (e) {
		...
		$.post('<?php echo base_url()?>admin/jens_validation', {
			...
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/edit_je', {
							'id': id,
							'name': name,
						},
						function (data) {
		...
		$.post('<?php echo base_url();?>admin/edit_je', {'id': id, 'name': name},
			function (data) {
				if (data == 1) {
					$("#name_" + id).text(name);
