		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<label class="required">مدل:</label><br>
					<input required autocomplete="off" id="name" class="form-control" type="text" placeholder="مدل">
			...
			<div class="modal-content " style="background-color: #fff">
				...
				<div class="modal-body">
					<div>
						...
						<label class="required">مدل:</label><br>
						<input required autocomplete="off" id="name_modal" type="text" class="form-control" placeholder="مدل">
...
<script type="text/javascript" language="javascript">
	$(document).ready(function () {
		var dataTable = $('#model_data').DataTable({
			...
			"order": [],
			"ajax": {
				url: "<?php echo base_url() . 'admin/model_list'; ?>",
				type: "POST"
			},
			"columnDefs": [
				{
		...
		$('#name').keyup(function () {
			...
			function load_data(query) {
				$.ajax({
					url: "<?php echo base_url(); ?>admin/search_model",
					method: "POST",
					data: {query: query,},
			...
			if (name != '') {
				load_data(name);
				$.ajax({
					url: "<?php echo base_url(); ?>admin/check_model",
					method: "POST",
					data: {name: name},
					success: function (data) {
						$('#error').html(data);
	...
	$(document).on('click', '#delete', function () {
		var id = $(this).attr("id_model");
		$.ajax({
			url: "<?php echo base_url(); ?>admin/check_model_in_tables",
			method: "POST",
			data: {id: id},
			success: function (data) {
				...
				if (m2 == true) {
					$.ajax({
						url: "<?php echo base_url(); ?>admin/delete_model",
						method: "POST",
						data: {id: id},
						success: function (data) {
							if(data.includes(1)){
								$('#model_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#insert', function (e) {
		...
		$.post('<?php echo base_url()?>admin/model_validation', {
				'name': name,
			},
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/insert_model', {
							'name': name,
						...
						function (data) {
							if (data.includes(1)) {
								...
								//window.location.href = "<?php echo base_url(); ?>admin/model";
	...
	$(document).on('click', '#copy', function (e) {
		var id = $(this).attr("id_model");
		$.post('<?php echo base_url()?>admin/model_copy', {
				'id': id,
			},
			function () {
				$('#model_data').DataTable().ajax.reload( null, false );
	...
	$(document).on('click', '#btn_edit', function (e) {
		...
		$.post('<?php echo base_url()?>admin/model_validation', {
			...
			function (data) {
				if (data.includes(1)) {
					$.post('<?php echo base_url()?>admin/edit_mo', {
							'id': id,
							'name': name,
						},
						function (data) {
		...
		$.post('<?php echo base_url();?>admin/edit_mo', {'id': id, 'name': name},
			function (data) {
				if (data == 1) {
					$("#name_" + id).text(name);
