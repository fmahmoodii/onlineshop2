...
(function ($) {
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            _cw = parseInt(options.cellWidth)+10;
            _ch = parseInt(options.cellHeight);
            self.cellStyle = "style='width:" + _cw + "px;height:" + _ch + "px;line-height:" + _ch + "px; font-size:" + (_fontSize) + "px; ' ";
            self.headerStyle = "style='height:" + _ch + "px;line-height:" + _ch + "px; font-size:" + (_fontSize + 4) + "px;padding-bottom: 40px;' ";
        ...
        persianDatepicker.prototype = {
            ...
            dows: function () {
                ...
                for (i = 0; i < 7; i++) {
                    if(i==0){
                    _cell = $('<div class="dow cell fst" ' + this.cellStyle + ' />').html(this.options.shortDowTitle[i]);}
                    else{
            ...
            content: function () {
                ...
                for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
                    _row = $('<div class="pbm10" />');
                    for (var col = 0; col < 7; col++ , cellIndex++) {
                        if (cellIndex < _start || cellIndex - _start + 1 > _end) {
                            if(col==0){_cell = $('<div class="nul cell fst " ' + self.cellStyle + ' />')
                                .html('&nbsp;');}
                            else {_cell = $('<div class="nul cell " ' + self.cellStyle + ' />')
                                .html('&nbsp;');}
                        } else {
                            _dt = self.getDate(self.persianDate, cellIndex - _start + 1);
						
                            _today = '', _selday = '', _disday = '' , _dddd = '';
						...
						if (self.options.startDate != null && (self.persianDate.parse(self.options.startDate).compare(_dt) == -1 || self.persianDate.parse(self.options.endDate).compare(_dt) == 1))
                            ...
                            if (self.options.selectedDate != undefined) {
                                ...
                                _selday = 'selday';
                            _fri = col == 6 ? 'friday lst' : '';
                            _sat = col == 0 ? 'fst' : '';
							var xyx= 0;
			if (self.options.blockdays != null) {
				
               var bd= self.options.blockdays;
			   bd= bd.split(',');
			...
			if(_dt==bd[i]){
				xyx=1;
						...
						if (self.options.reservdays != null) {
				
               var rd= self.options.reservdays;
			   rd= rd.split(',');
			...
			if(_dt==rd[i]){
				xyx=1;
                            ...
                            _cell = $('<div class="day cell ' + _fri + ' '+ _sat + ' ' + _today + ' ' + _selday + ' ' + _dddd + ' ' + _disday + '" ' + self.cellStyle + ' />');
